...
A SOLIDWORKS macro that switches the interface to the Dark brightness theme from
a toolbar button, instead of clicking through **Tools > Options > System Options
> Colors** every time.
...
Applies the **Dark** interface brightness theme, which covers the
CommandManager, FeatureManager, menus and task pane. The graphics area behind
your model is a separate setting — pair this with [Set Background - Colour
Light](https://github.com/james-debono/set-background-colour-light-sw-macro) for
a dark interface around a white graphics area, which is the usual combination
for screenshots.
...
**The macro on its own:** download `Set-UI-Theme-Dark.swp` from the [latest
...
- [Set UI Theme - Light](https://github.com/james-debono/set-ui-theme-light-sw-macro)

## Building from source

`src/Set-UI-Theme-Dark.vba` is the source. A `.swp` is a binary VBA project, so
